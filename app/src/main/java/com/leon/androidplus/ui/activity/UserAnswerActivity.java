...
public class UserAnswerActivity extends BaseListViewActivity<Answer> {
    ...
    protected void startRefresh() {
        mAnswerDataSource.refreshUserAnswerList(mUserId, new LoadCallback<Answer>() {
            @Override
    ...
    protected void startLoadMoreData() {
        mAnswerDataSource.getMoreUserAnswerList(mUserId, new LoadCallback<Answer>() {
            @Override
            public void onLoadSuccess(List<Answer> list) {
    ...
    protected void startLoadData() {
        initUser();
        mAnswerDataSource.getUserAnswerList(mUserId, new LoadCallback<Answer>() {
            @Override
            public void onLoadSuccess(List<Answer> list) {
                mSwipeRefreshLayout.setRefreshing(false);
                getAdapter().replaceData(list);
    ...
    BaseListAdapter<Answer> onCreateAdapter() {
        return new UserAnswerAdapter(this);
    }
